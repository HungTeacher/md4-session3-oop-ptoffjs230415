...
const result = func("hung")
console.log("result: ", result)

/*Object*/
const person: { name: string } = {name: "Tony"}
...
const countChar1 = charCount("hello")
...
/*example about union & alias type*/

/*type alias with object*/
type User = { name: string, age: number }
const u1: User = {name: 'M1', age: 18}
...
let combine1 = combine(10, 20);
console.log("example union type number: ", combine1)
let combine2 = combine("Hello ", "Cac ban")
console.log("example union type string: ", combine2)

/*Function Type*/
function add(a: number, b: number, cb: (result: number) => void): void {
    let result = a + b;
    cb(result)
}

let addResult = add(10, 20, (result) => console.log(result))
...
*/
function getFullNameUsers(users: Users): string[] {
    return users.map(user => `${user.first_name} ${user.last_name}`)
}

const fullNameUserArr = getFullNameUsers(users)
...
function sortUserByFirstName(users: Users): Users {
    const sortedUsers = [...users];
    sortedUsers.sort((a, b) => a.first_name.localeCompare(b.first_name))
    return sortedUsers
}
const sortedUserbyFirstName = sortUserByFirstName(users)
